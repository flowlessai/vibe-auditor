...
import { runCommand, writeDiffContent } from "./project.js";
import { c, line, write } from "./ui.js";
function asString(value) {
    return typeof value === "string" && value.trim() ? value : undefined;
}
function toSafePath(raw) {
    return String(raw ?? "").trim().replace(/^([ab]\/)+/, "").replace(/^\/+/, "");
}
function detectPathFromPatch(patch) {
...
function extractPath(obj) {
    ...
    for (const candidate of candidates) {
        ...
        if (str)
            return toSafePath(str);
    }
    return undefined;
}
function extractPatch(obj) {
    const candidates = [
        obj.patch,
        obj.diff,
        obj.unifiedDiff,
...
function extractNewContent(obj) {
    const candidates = [
        obj.newContent,
        obj.content,
        obj.after,
...
function collectDiffObjects(rawDiffs) {
    const queue = [...rawDiffs];
    const result = [];
...
export function extractDiffs(rawDiffs) {
    const out = [];
    const seen = new Set();
...
async function applyDiffs(projectDir, diffs) {
    const patches = diffs.map((d) => d.patch).filter((v) => Boolean(v));
    if (patches.length > 0) {
    ...
    for (const diff of diffs) {
        if (!diff.newContent || diff.path === "unknown-file")
            continue;
        await writeDiffContent(projectDir, diff.path, diff.newContent);
    }
}
export async function reviewDiffsInteractively(projectDir, diffs) {
    if (diffs.length === 0) {
    ...
    try {
        for (let i = 0; i < diffs.length; i++) {
            ...
            console.log("");
            const answer = await rl.question(`  ${c.bold}Apply this fix?${c.reset}  ${c.gray}[${c.reset}${c.brightGreen}y${c.reset}${c.gray}/${c.reset}${c.brightRed}n${c.reset}${c.gray}/${c.reset}${c.yellow}q${c.reset}${c.gray}]${c.reset}  `);
