...
import { artifactPreviewLines, printArtifacts, printGlobalAnalysis } from "./artifacts.js";
import { reviewOutputArchiveAndApply } from "./output-review.js";
import { DOTS, PULSE, SPINNER, STATUS_LABEL, c, cls, hideCursor, line, showCursor, sleep, statusBadge, termWidth, write } from "./ui.js";
import { POLL_INTERVAL_MS } from "./config.js";
let lastRenderedLines = 0;
export function resetLiveRender() {
    lastRenderedLines = 0;
}
function renderLive(task, projectName, elapsedMs, frame) {
...
export async function monitorTask(taskId, token, projectDir, projectName) {
    ...
    const onSigint = async () => {
        ...
        try {
            await request("POST", `/pause/${taskId}`, { token });
