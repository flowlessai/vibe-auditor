import { c, line, severityBadge, write } from "./ui.js";
...
export function artifactPreviewLines(artifacts, limit = 3) {
...
export function printArtifacts(artifacts) {
    if (artifacts.length === 0)
        return;
    console.log("");
    ...
    for (const artifact of artifacts.map(normalizeArtifact)) {
        ...
        if (artifact.severity)
            write(`  ${severityBadge(artifact.severity)}`);
        write("\n");
...
export function printGlobalAnalysis(analysis) {
    if (!analysis)
        return;
    console.log(`  ${c.bold}${c.brightWhite}GLOBAL ANALYSIS${c.reset}`);
    console.log(`  ${line()}`);
    console.log("");
    const maybeObject = typeof analysis === "object" ? analysis : null;
